...
BOOL is_udp_packet(void *buffer,u_int_t len,UDP *packet)
{
    ...
    if(is_ip_packet(buffer,len,&ip_header))
    {
            ...
            if(packet->checksum != 0x0000) //if udp checksum is set
               if(packet->checksum != udp_checksum(packet,ip_header.src,ip_header.dst)) //check the checksum
                        return FALSE;
...
u_int16_t udp_checksum(UDP *udp,u_char_t *src_ip,u_char_t *dst_ip) 
{
   ...
   if(temp != 0x0000)
               udp->checksum = 0xFFFF; // set it to be zero
   else
               udp->checksum = 0x0000;
