...
u_int_t create_arp_packet(u_char_t *ip_to, u_char_t *eth_to, u_char_t *host_ip, u_char_t *host_mac, u_int16_t arp_op, ARP packet) {
    ...
    packet->arp_proto_size = IP_LEN;
    packet->arp_op = htons_tos(arp_op);
    memcpy_tos(packet->arp_eth_source, host_mac, ETH_ADDR_LEN);
    memcpy_tos(packet->arp_ip_source, host_ip, IP_LEN);
    memcpy_tos(packet->arp_eth_dest, eth_to, ETH_ADDR_LEN);
    memcpy_tos(packet->arp_ip_dest, ip_to, IP_LEN);
